...
if settings.sensors.ds18b20.enable :
    ...
    results.append( {
            ...
            "customunit": "°C",
            "float": 1,
            "value": float(format(ds18.temperature, "0.4f"))
        })
...
if settings.sensors.sht4x.enable :
    ...
    results.append( {
        ...
        "customunit": "°C",
        "float": 1,
        "value": float(format(sht.measurements.temperature, "0.4f"))
    })
    results.append({
        "channel": "Humidity (SHT4x)",
        "customunit": "%",
        "float": 1,
        "value": float(format(sht.measurements.relative_humidity, "0.4f"))
    })
